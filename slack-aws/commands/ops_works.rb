        ...
        when 'instance' then
          ...
          case instance_cmd
            ...
            when 'provision' then
              ...
              fail "<from> cannot be empty.  Use syntax `<stack>:<instance>` to specify which instance to clone from, or `empty:empty` or `empty:legacyempty` to provision with an empty db." if !from_stack || from_stack.empty?
              ...
              elsif from_stack == "empty" and from_instance == "legacyempty"
                deploy_response = opsworks_client.create_deployment(stack_id: @@current_stack_id, instance_ids:[instance.instance_id], command: { name: 'execute_recipes', args: { recipes: ["soxhub::provision","soxhub::empty_db"] }}, custom_json:"{\"soxhub\": { \"provision\": { \"instances\": { \"#{provision_hostname}\": true },\"api_branch\": \"#{api_branch}\",\"client_branch\": \"#{client_branch}\"}, \"empty_db\": { \"legacy\": true, \"instances\": { \"#{provision_hostname}\": true } }}}")
            ...
            when 'clonedb' then
              ...
              commands = opsworks_client.describe_commands(instance_id: instance.instance_id).commands
              fail "another command is currently running.  please wait for the prior command to complete before cloning.  the prior command is in status *#{commands[0].status}*" if commands.size && commands[0].status != "successful" && commands[0].status != "failed"
              ...
              db_response = opsworks_client.create_deployment(stack_id: @@current_stack_id, instance_ids:[instance.instance_id], command: { name: 'execute_recipes', args: { recipes: ["soxhub::clone_db"] }}, custom_json:"{\"soxhub\": { \"clone_db\": { \"instances\": { \"#{hostname}\": true }, \"from\": { \"stack\": \"#{from_stack}\", \"instance\":\"#{from_instance}\" } }}}")
            ...
            when 'restoredb' then
              ...
              send_message client, data.channel, "RESTORING DB FROM FILE: `#{sql_file}`"
              send_message client, data.channel, "instance: *#{hostname}*, stack: *#{@@current_stack}*"
              send_message client, data.channel, "use `aws ops instance status #{hostname}` or login to opsworks to view the status of this operation."
            ...
            when 'emptydb' then
              ...
              commands = opsworks_client.describe_commands(instance_id: instance.instance_id).commands
              fail "another command is currently running.  please wait for the prior command to complete before emptying.  the prior command is in status *#{commands[0].status}*" if commands.size && commands[0].status != "successful" && commands[0].status != "failed"

              db_response = opsworks_client.create_deployment(stack_id: @@current_stack_id, instance_ids:[instance.instance_id], command: { name: 'execute_recipes', args: { recipes: ["soxhub::empty_db"] }}, custom_json:"{\"soxhub\": { \"empty_db\": { \"instances\": { \"#{hostname}\": true } }}}")

              send_message client, data.channel, "EMPTY DB operation started!"
              send_message client, data.channel, "instance: *#{hostname}*, stack: *#{@@current_stack}*"
              send_message client, data.channel, "use `aws ops instance status #{hostname}` or login to opsworks to view the status of this operation."

            when 'legacyemptydb' then
              hostname = arguments.shift
              fail 'Invalid instance name.  Use `aws ops instance ls` to see available instances in stack *#{@@current_stack}*.' unless hostname
              ...
              db_response = opsworks_client.create_deployment(stack_id: @@current_stack_id, instance_ids:[instance.instance_id], command: { name: 'execute_recipes', args: { recipes: ["soxhub::empty_db"] }}, custom_json:"{\"soxhub\": { \"empty_db\": { \"legacy\": true, \"instances\": { \"#{hostname}\": true } }}}")

              send_message client, data.channel, "LEGACY EMPTY DB operation started!"
              send_message client, data.channel, "instance: *#{hostname}*, stack: *#{@@current_stack}*"
              send_message client, data.channel, "use `aws ops instance status #{hostname}` or login to opsworks to view the status of this operation."
